...
jQuery(function($){
    ...
    // $("#all").css({"width":width,"height":"568px"});
    var f = 640/1008;
    var win = {
        h : $(window).height(),
        w : $(window).width()
    ...
    $('.endbtn').on('touchstart',function(){
        ...
        if(number != number2){
            ...
            if(tar == "sub"){
                ...
                var _city = document.getElementById("province");
                var _qu = document.getElementById("city");
                var _dealer = document.getElementById("dealers");
                var dealer = _dealer.options[_dealer.selectedIndex].text;
                if( mobile == ''){
                ...
                DomobLanding.monitor.reg({
                    ...
                    sex:sex,
                    mobilenumber:mobile,
                    sheng:_city.options[_city.selectedIndex].text,
                    city:_qu.options[_qu.selectedIndex].text,
                    dealer:_dealer.options[_dealer.selectedIndex].text,
                    cartype:cartype
                });
                //number2 = _mobilenumber.value;
                alert('提交成功，谢谢！');
                $('#name').val('姓名');
    ...
    //loading Dom
    action.page2.p2wbg();
    setTimeout("$('#home_drag').addClass('action');$('.bar_wrap').show()",500);
})
...
var img = new Image();      // 澹版槑瀵硅薄锛屼娇鐢� image 瀵硅薄涓殑 onload 鏂规硶
